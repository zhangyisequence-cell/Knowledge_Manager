...
**当前：安装已启动，系统分区和基础文件已写入；正在下载内核及固件，尚未验证安装结束和目标系统登录。** 安装环境地址为 `172.27.65.219`，可经宿主机转发进行公钥登录。2026-09-19 实查同一 `apt-get ... --download-only linux-generic` 进程仍在运行，下载从 Intel、Marvell 固件推进到 NVIDIA 固件；期间未重新安装或重启。这个地址仍属于临时安装系统，不能当作最终部署成功的证据。
