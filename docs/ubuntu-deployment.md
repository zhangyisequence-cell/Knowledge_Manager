...
## 最新状态（2026-09-19 05:27）

当前应用 release 为 `/opt/knowledge-manager/releases/a849d7f`，05:23:35 更新单元成功，健康接口 `status=ok`、`storage_configured=true`、AI 关闭。新增的自动转写来源与复核提示已随该版本部署；实际 HTTP 媒体入库复验正在准备，不能据此声称转写准确率达标。
...
Whisper 固定快照已离线校验并发布到服务账户的模型缓存。旧 CPU 的实际三格式转写耗时 152.63 秒、峰值约 1 GiB、无 swap；严格事实检查仍每项 4/5，“归档”识别错误。llama.cpp 固定源码已于 05:22:55 编译成功，版本命令能够运行，动态库均可解析；源码归档没有 Git 元数据，因此版本输出为 `0.4.1-dev / build 0 / commit unknown`，固定 commit 由已校验的归档 SHA 与安装标记追溯。Qwen 模型仍在下载，尚无真实 AI 质量或推理资源验收。

下文保留各阶段的历史事实；较早的“待配置/尚未完成”状态以本节和最新验证记录为准。微信账号接入、远程 Obsidian 客户端使用、整机重启和 AI 实际验收仍待完成。
