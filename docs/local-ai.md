# Archived local CPU AI service

This document records the former Qwen/llama.cpp experiment. It is retained for provenance and recovery only; the service is disabled and is not the production analysis provider. The approved production path sends extracted正文、表格文本、OCR 文本和音视频转写文本 to DeepSeek through its OpenAI-compatible API. Original binary media remains local and is never uploaded by the application.

Do not create `/etc/knowledge-manager/local-ai.enabled` or enable the local LLM unit as part of the current deployment. The application uses `KNOWLEDGE_OPENAI_BASE_URL=https://api.deepseek.com`, `OPENAI_MODEL=deepseek-chat`, and a locally stored `OPENAI_API_KEY` only after grounded-ingestion acceptance passes. `AI_ENABLED=false` remains the repository default.
...
Those checks apply only to the archived local service. They do not enable application AI. The current provider configuration is:

- base URL: `https://api.deepseek.com`
- model: `deepseek-chat`
- API key: stored only in Ubuntu local configuration
- vision model: unset; all media is first extracted/transcribed locally
...
The user has explicitly authorized sending extracted text to DeepSeek. Keep API keys, original binaries, and real knowledge out of Git and logs.
