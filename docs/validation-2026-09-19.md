...
当前部署源码的本地提交为 `2a76c089e3c1c773b291f129911d4ddd000b5f93`，远端提交为 `5cafe99bb70706a7e14fe37a16a07a263e2ff45a`，两者 tree 均为 `36762501b1edee5e33d006bf6d08f256eb4da330`；对应的两次最新 CI 均成功。这里不据此改写上方统一测试总数，因为本轮没有重新运行统一测试。
...
新增 `scripts/check_server_ingestion.py`，用于检查文本、TXT、Markdown、DOCX、XLSX、PDF 和旧 DOC 七类真实服务器入库，包括任务终态、原始正文、文件模式以及原件和附件 SHA；脚本尚未在正式服务器运行。

Ubuntu 24.04.4 已完成安装、从系统盘独立启动，并在正常关机调整为固定 4 GiB 内存、2 vCPU 后重新验证 SSH。应用安装仍在 apt 下载阶段，尚无健康检查结果。仍需完成正式应用部署、服务器中文模型/OCR/AI、七类真实入库、微信端到端收发、远程 Obsidian 使用、正式备份恢复和第二备份介质。不能据本地测试、CI 或基础系统就绪将这些项目标记为完成。
