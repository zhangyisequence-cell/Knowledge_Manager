...
def get_config() -> AppConfig:
    ...
    if value := os.getenv("OBSIDIAN_VAULT_DIR"):
        raw["vault_dir"] = value
    # Read provider settings once so a request cannot observe a partially changed
    # environment. Secrets are kept in memory only and are never represented.
    env_base_url = os.getenv("KNOWLEDGE_OPENAI_BASE_URL") or os.getenv("OPENAI_BASE_URL")
    env_api_key = os.getenv("OPENAI_API_KEY")
    env_model = os.getenv("OPENAI_MODEL")
    env_enabled = os.getenv("AI_ENABLED")
    if env_base_url:
        raw.setdefault("ai", {})["base_url"] = env_base_url
    if env_api_key:
        raw.setdefault("ai", {})["api_key"] = env_api_key
    if env_model:
        raw.setdefault("ai", {})["model"] = env_model
    if value := os.getenv("OPENAI_VISION_MODEL"):
        raw.setdefault("ai", {})["vision_model"] = value
    if env_enabled:
        raw.setdefault("ai", {})["enabled"] = env_enabled.lower() == "true"
